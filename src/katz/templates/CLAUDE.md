...
2. Confirm the canonical manuscript before registration when more than one
   ...
   audit. Text dependencies must stay inside the repository; sibling-directory
   graphics are permitted but recorded as external assets. Never pass
   `--allow-lossy` for missing media without user approval.
